...
/**
 * @file
 * Theme setting callbacks for the md_delphinus theme.
 */
global $base_url;
define('THEME_PATH', drupal_get_path('theme', 'md_delphinus'));
define('CURRENT_THEME', variable_get('theme_default'));
define('BASE_THEME', 'md_delphinus');

require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'md_delphinus') . '/inc/admin/utilities.inc';
require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'md_delphinus') . '/inc/admin/form.inc';
require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'md_delphinus') . '/inc/admin/theme-settings-general.inc';
require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'md_delphinus') . '/inc/admin/theme-settings-design.inc';
require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'md_delphinus') . '/inc/admin/theme-settings-text.inc';
require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'md_delphinus') . '/inc/admin/theme-settings-pages.inc';
require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'md_delphinus') . '/inc/admin/theme-settings-code.inc';
require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'md_delphinus') . '/inc/admin/theme-settings-config.inc';
...
function md_delphinus_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL, $no_js_use = FALSE) {
  global $base_url;
  $path = drupal_get_path('theme', 'md_delphinus');
  ...
  $form['md_delphinus_settings']['html_header'] = array(
  ...
  md_delphinus_theme_settings_general($form, $form_state);
  md_delphinus_theme_settings_design($form, $form_state);
  md_delphinus_theme_settings_text($form, $form_state);
  md_delphinus_theme_settings_code($form, $form_state);
  md_delphinus_theme_settings_config($form, $form_state);


  $form['actions']['reset'] = array(
    '#type' => 'submit',
    '#value' => t('Reset Settings'),
    '#submit' => array('md_delphinus_reset_settings_submit'),
    '#weight' => 98,
    '#attributes' => array(
 ...
 */
function md_delphinus_reset_settings_submit($form, &$form_state) {
  $theme_settings = variable_get('theme_' . variable_get('theme_default') . '_settings');
  $default_settings = _md_delphinus_theme_default_settings($theme_settings);
 ...
 */
function md_delphinus_restore_theme_settings($form, &$form_state) {
 ...
 */
function _md_delphinus_theme_default_settings($theme_settings) {
